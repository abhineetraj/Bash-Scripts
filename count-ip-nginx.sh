...
for (( i=$zero; i<=$hrs; i++ ))
do
    grep `date -d now-${i}hours  +%d/%b/%Y:%H` /var/log/nginx/access.log >> /tmp/updated_logs
done

#Calculate the ip's
awk '{print $1}' /tmp/updated_logs > /tmp/updated_ips
printf "Logs are stored @ /tmp/updated_logs\n" && sleep 2
printf "Printing the count of IP's for the last $hrs hours \n"
source /var/local/adm/progress.sh && progressbar 5
sort /tmp/updated_ips | uniq -c | sort -n -r -k1,1 | head -40
